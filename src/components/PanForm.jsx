...
const SimplePanCardForm = () => {
  ...
  const extractTextFromImage = async (imageBase64) => {
    ...
    try {
      ...
      const mockExtractedData = {
        ...
        nameAsPerPanCard: 'KAUSHAL SURESH PATEL',
      }

      // Fill form fields
      setFormData(mockExtractedData)

      alert('PAN card information extracted successfully! (Using mock data for testing)')
    } catch (error) {
      console.error('Error with mock extraction:', error)
      alert('Failed to extract PAN card information.')
    } finally {
    ...
    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [
              {
                text: "Extract all information from this PAN card document and return it as a JSON object with the following fields: panNumber, nameAsPerPanCard. Only return the JSON object, no other text."
              },
              {
                inline_data: {
      ...
      if (data.candidates && data.candidates[0] && data.candidates[0].content) {
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            alert('PAN card information extracted successfully!');
          } else {
            throw new Error('No JSON found in response');
          }
        } catch (parseError) {
          console.error('Failed to parse extracted data:', parseError);
          alert('Failed to parse PAN card information. Please check the image quality.');
        }
      } else {
      ...
      alert('Failed to extract PAN card information. Please check your API key and try again.');
    } finally {
      setIsProcessing(false);
    }
    */
  ...
  const handleSubmit = () => {
    ...
    console.log('PAN Card Form submitted:', submissionData)
    alert(`PAN card form submitted successfully!\n\nData:\n${JSON.stringify(formData, null, 2)}`)
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-6">
            <h1 className="text-3xl font-bold text-white">Upload PAN Card</h1>
          </div>
          ...
          <div className="p-8">
            <div className="grid lg:grid-cols-2 gap-8">
              ...
              <div>
                <h3 className="text-xl font-semibold text-gray-800 mb-4">PAN Card Image</h3>
              ...
              <div>
                <div className="space-y-6 mt-10">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Name as per PAN Card</label>
                    <input
                      type="text"
                      value={formData.nameAsPerPanCard}
                      onChange={(e) => handleInputChange('nameAsPerPanCard', e.target.value)}
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div className="flex mt-6 gap-4 justify-end">
                    <button type="button" onClick={() => navigate('/aadhaar')} className="btn btn-secondary p-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-4 rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-colors font-medium text-lg shadow-lg">
                      Previous
                    </button>
                    <button type="submit"
...
export default SimplePanCardForm
